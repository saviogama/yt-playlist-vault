...
public class SnapshotService {
    ...
    private MonitoredPlaylist getPlaylistOrThrow(UUID playlistId) {
        return monitoredPlaylistRepository.findById(playlistId)
    ...
    public SnapshotWithItems createSnapshot(UUID playlistId, CreateSnapshotRequest request) {
        ...
        List<SnapshotItem> items = request.items()
                .stream()
    ...
    public List<SnapshotWithItems> listSnapshots(UUID playlistId) {
        ...
        return snapshotRepository.findByMonitoredPlaylistOrderByCapturedAtDesc(playlist)
                .stream()
                .map(snapshot -> new SnapshotWithItems(
                        snapshot,
                        snapshotItemRepository.findBySnapshotOrderByPositionAsc(snapshot)
                ))
                .toList();
    }
    ...
    public record SnapshotWithItems(
            Snapshot snapshot,
